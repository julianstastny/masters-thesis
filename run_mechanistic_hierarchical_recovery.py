...
from numpyro.handlers import reparam, trace, seed
...
#         os.makedirs(f"output/{name}/idatas")
    ...
    samples = sample_from_model(
        hierarchical_model, 
        {}, 
        z_sample=['alpha', 'lapse_prob', 'approach_given_lapse', 'stimulated_weights', 'repetition_kernel', 'true_weight_mean', 'true_weight_mean_prior_mean', 'true_weight_mean_prior_scale', 'learning_rate', 'delta'] + [f'{i}_y' for i in str_dates] + [f'{i}_logits' for i in str_dates] + [f'{i}_repetition_kernel' for i in str_dates] + [f'{i}_probs_with_lapse' for i in str_dates] + [f'{i}_delta' for i in str_dates] +  [f"{i}_AR(1) with learning" for i in str_dates], 
        num_samples=1, start_seed=1, X_sep=X_sep, stage_sep=stage_sep, y_sep=[None] * NUM_SESSIONS, str_dates=str_dates)
    
    with open(f'output/{name}/samples.pkl', 'wb') as f:
        pickle.dump(samples, f, pickle.HIGHEST_PROTOCOL)
    print("Samples generated and saved")
    
    y_sep_sample = [samples[f'{i}_y'][0] for i in str_dates]

    if smoke_test:
        mcmc, idata = fit(hierarchical_model, 8, X_sep=X_sep[:2], y_sep=y_sep_sample[:2], stage_sep=stage_sep[:2], str_dates=str_dates[:2])
    else:
        mcmc, idata = fit(hierarchical_model, 8, X_sep=X_sep, y_sep=y_sep_sample, stage_sep=stage_sep, str_dates=str_dates)
...
#     run(config, 'mechanistic_parameter_recovery', smoke_test=args.smoketest)
    #
    # config = copy.deepcopy(base_config)
